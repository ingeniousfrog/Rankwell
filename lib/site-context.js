...
const buildCrawlFailureHint = (attemptedUrls = []) => {
...
const createSiteContextForStartUrl = async ({ startUrl, fetchImpl, limits, onProgress }) => {
  ...
  return {
    ok: pages.length > 0,
    error: pages.length > 0 ? "" : "No crawlable HTML pages were fetched.",
    startUrl,
...
export const createSiteContext = async ({ url, fetchImpl = fetch, limits = {}, onProgress }) => {
  ...
  return {
    ...
    error: `${lastResult.error}${buildCrawlFailureHint(candidates)}`,
  };
};
