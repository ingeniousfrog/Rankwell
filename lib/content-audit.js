...
export const auditCalendarItem = (item, inputs = {}, siteContext = null) => {
  const checks = [];
  const isOpportunityTask = ["refreshBrief", "expandBrief", "governance"].includes(item.draftMode);
  const titleScore = isOpportunityTask ? { ok: Boolean(item.title), reason: "Opportunity task title is workflow-facing." } : scoreAudienceTitle(item.title, inputs);
  if (isOpportunityTask) {
    checks.push(
      qa(
        "Opportunity task",
        item.title ? "pass" : "fail",
        item.title
          ? "Task title can describe the work because this plan item is not a new article headline."
          : "Opportunity task is missing a title.",
      ),
    );
  } else if (!titleScore.ok) {
  ...
  if (isOpportunityTask && !item.targetUrl && item.draftMode !== "governance") {
    checks.push(qa("Target URL", "fail", "Refresh and expansion briefs must name the existing URL they update."));
  }
